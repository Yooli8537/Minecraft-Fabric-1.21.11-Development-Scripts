...
$testInstanceModPath = "$env:APPDATA\ModrinthApp\profiles\ModTest\mods"

if (!(Test-Path $testInstanceModPath)) {
    Write-Host "Couldn't find Modrinth Instance." -ForegroundColor Red
    Write-Host 'Make sure your Modrinth Instance Folder is named "ModTest".' -ForegroundColor Cyan
    exit 1
}
